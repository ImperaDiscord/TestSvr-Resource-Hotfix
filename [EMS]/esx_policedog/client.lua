...
RegisterNetEvent('esx_policedog:hasItems')
AddEventHandler('esx_policedog:hasItems', function(hadIt)
    if hadIt then
        ESX.ShowNotification(Strings['items_found'])
        loadDict('missfra0_chop_find')
        TaskPlayAnim(policeDog, 'missfra0_chop_find', 'chop_bark_at_ballas', 8.0, -8, -1, 0, 0, false, false, false)
    else
        ESX.ShowNotification(Strings['no_items'])
    end
end)
    ...
    if PlayerData.job.name == Config.Job[1] or PlayerData.job.name == Config.Job[2] then
        ...
        ESX.UI.Menu.Open(
                'default', GetCurrentResourceName(), 'buy_storage',
            {
                ...
                align = 'top-left',
                elements = {{label = Strings['take_out_remove'], value = 'take_out_remove'}, {label = Strings['small'], value = 'small'}}
            },
            function(data, menu)
                        ...
                        ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
                            if quantity > 0 then
                                ...
                                if health > 0 then
                                    ...
                                    IsBusy = true
                                    ESX.ShowNotification(('~g~Vous soigner le Chien'))
